          ...
          <div class="row">
            <div class="col-xl-12">
              <div class="text-end mb-20">
                <NuxtLink to="/video/add" class="btn btn-success">
                  <i class="fa fa-plus" />
                  เพิ่มวิดีโอ
                </NuxtLink>
              </div>
                ...
                <tbody>
                  <tr v-for="(it, index) in items" :key="index">
                    ...
                    <td class="text-center">
                      <img :src="it.video_file" style="width: 100px" alt="" />
                    </td>
                    <td>{{ it.title }}</td>
                    <td class="text-center">
                      {{
                        dayjs(it.created_video).locale("th").format("DD MMM BB")
                      }}
                    </td>

                    <td class="text-center">
                      <NuxtLink :href="it.video_url" target="_blank">
...
const onDelete = async (id) => {
  await $fetch(`${runtimeConfig.public.apiBase}/video/${id}`, {
    method: "DELETE",
  })
    .then((res) => {
